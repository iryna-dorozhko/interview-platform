...
1. **Якщо exit code не 0** — проаналізуй вивід:
...
2. **Цикл** — повторюй кроки 1–2, доки **`npx @7n/rules lint`** не завершиться успішно.

3. **Верифікація** — фінальна перевірка (обов'язково з кодом **0**):

```bash
npx @7n/rules lint
```

1. **Результат** — коротко опиши, що саме виправлено; якщо щось блокує нульовий exit code — залиш чітке пояснення й наступні кроки для людини.
