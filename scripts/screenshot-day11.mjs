...
async function main() {
  ...
  // 4) HR: live room + arbiter process feed (inject entries for UI proof)
  {
    ...
    await page.waitForSelector(".agent-panel, [aria-label='Статус AI-агентів']", {
      timeout: 20_000
    })
  ...
  // 5) Candidate: contact bootstrap — open chat and wait for name prompt
  {
    ...
    await page.waitForSelector('button:has-text("Створити анкету"), button:has-text("Продовжити анкету")', {
      timeout: 20_000
    })
    await page.click('button:has-text("Створити анкету"), button:has-text("Продовжити анкету")')
    await page.waitForFunction(() => /звати|ім'я|Ім'я|Привіт|контакт/i.test(document.body?.textContent ?? ''), null, {
      timeout: 90_000
    })
