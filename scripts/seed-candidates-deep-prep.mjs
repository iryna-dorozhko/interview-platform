...
async function runCandidatePrep(token, interviewId, answers, label) {
  ...
  for (const [i, answer] of answers.entries()) {
    res = await apiWithRetry('POST', `/candidate-prep/${interviewId}/message`, {
      token,
      body: { message: answer }
    })
  ...
  if (!ready) {
    const extra = await apiWithRetry('POST', `/candidate-prep/${interviewId}/message`, {
      token,
      body: {
        message:
...
async function main() {
  ...
  for (const item of created) {
    console.log(
      `- ${item.label}: ${item.email} | interview=${item.interviewId} | confirmed=${Boolean(item.confirmedAt)}`
    )
  }
}
