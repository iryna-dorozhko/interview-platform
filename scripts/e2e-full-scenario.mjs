...
const HR_FALLBACK_PROFILE = {
  ...
  culture: ['плоска структура', 'remote-first', 'відкрита комунікація в Slack', 'code review'],
  expectations: [
...
const CANDIDATE_FALLBACK_PROFILE = {
  ...
  goals: ['senior у продуктовій команді', 'менторство', 'архітектурні навички'],
  summary: 'Backend-розробник з 3 роками досвіду у FinTech: платежі, звірка, TypeScript/Node/PostgreSQL.'
}
...
async function waitForAgentActivity(collector, { minAgents = 1, timeoutMs = 45000, settleMs = 8000 } = {}) {
  ...
  while (Date.now() - start < timeoutMs) {
    const agents = collector.messages.slice(before).filter(m => String(m.authorType || '').startsWith('AGENT_'))
