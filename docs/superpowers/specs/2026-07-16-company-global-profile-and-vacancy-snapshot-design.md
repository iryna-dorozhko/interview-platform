...
- Роут: `/company-profile` (або `/company-prep`)
- Доступ: HR, зі списку вакансій (кнопка/банер «Профіль компанії»)
- UX: аналогічний `VacancyPrepView` — чат → finish → прев'ю профілю → confirm
- Після confirm: read-only, з позначкою «✓ Профіль компанії підтверджено»
...
- Банер + CTA на `VacancyPrepView` при `missingCompanyProfile`
- Форма редагування 8 полів перед підтвердженням вакансії
- `CompanyProfilePrepView` — повний цикл чат → finish → confirm
