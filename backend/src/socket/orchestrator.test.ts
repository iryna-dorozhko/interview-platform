...
function makePrisma(messages: LiveMessage[], statusOrOptions: 'LIVE' | 'READY' | MakePrismaOptions = 'LIVE') {
  const options: MakePrismaOptions =
    typeof statusOrOptions === 'string' ? { interviewStatus: statusOrOptions } : statusOrOptions
...
test('orchestrator COMPANY_ANSWER runs company with ANSWER_CANDIDATE', async () => {
  ...
  assert.equal((processEvents[0].payload as { action: string }).action, 'COMPANY_ANSWER')

  const agentMessages = emitted.filter(e => e.event === 'room:messages')
  assert.equal(agentMessages.length, 1)
  assert.equal(
    (agentMessages[0].payload as { messages: Array<{ authorType: string; content: string }> }).messages[0].authorType,
    'AGENT_COMPANY'
  )
...
test('orchestrator START posts arbiter, runs company, emits process', async () => {
  ...
  const processEvents = emitted.filter(e => e.event === 'room:arbiter-process')
  assert.ok(processEvents.length >= 1)
  assert.equal((processEvents[0].payload as { action: string; summaryUk: string }).action, 'START')
  assert.equal((processEvents[0].payload as { summaryUk: string }).summaryUk, 'Початок співбесіди')
...
test('orchestrator ANSWER runs only candidate', async () => {
  ...
  assert.equal(agentMessages.length, 1)
  assert.equal(
    (agentMessages[0].payload as { messages: Array<{ authorType: string }> }).messages[0].authorType,
    'AGENT_CANDIDATE'
  )
})
...
test('orchestrator WAIT runs nobody else and emits process', async () => {
  ...
  const orchestrator = createRoomOrchestrator(() => prisma, {
    debounceMs: 30,
    runArbiterTurn: async () => cmd({ action: 'WAIT', summaryUk: 'Розмова йде природно' }),
    runCompanyLiveTurn: async () => {
      companyCalls += 1
      return { post: false }
    },
...
test('superseded turn must not clear thinking while newer turn is still running', async () => {
  ...
  const orchestrator = createRoomOrchestrator(() => prisma, {
    ...
    runArbiterTurn: () => {
      ...
      if (arbiterCalls === 1) {
        return new Promise(resolve => {
          resolveFirst = () => resolve(cmd({ action: 'WAIT', summaryUk: 'stale' }))
        })
      }
      return new Promise(resolve => {
        resolveSecond = () => resolve(cmd({ action: 'WAIT', summaryUk: 'active' }))
      })
    }
  })
  ...
  const thinkingAfterStaleSettled = emitted.toReversed().find(e => e.event === 'room:agent-thinking')
...
test('orchestrator does not stop after second Company question before Candidate can ANSWER', async () => {
  ...
  assert.ok(
    callOrder.includes('candidate') && callOrder.filter(c => c === 'candidate').length >= 2,
  ...
  const candidateAfterSecond = emitted.filter(
    e =>
      e.event === 'room:messages' &&
      (e.payload as { messages: Array<{ authorType: string }> }).messages.some(m => m.authorType === 'AGENT_CANDIDATE')
  )
...
test('orchestrator conductor: NEXT_QUESTION then ANSWER in one loop', async () => {
  ...
  const authors = emitted
    ...
    .map(e => (e.payload as { messages: Array<{ authorType: string }> }).messages[0].authorType)
...
test('orchestrator stops after Candidate needsHuman (no deferral spam)', async () => {
  ...
  const agentMessages = emitted.filter(e => e.event === 'room:messages')
  assert.equal(agentMessages.length, 1)
  assert.match((agentMessages[0].payload as { messages: Array<{ content: string }> }).messages[0].content, /Ірино/)
})
...
test('orchestrator arbiter failure emits agent-error; onAgentRetry resumes arbiter', async () => {
  ...
  assert.equal((errors[0].payload as { agentType: string; error: string }).agentType, 'AGENT_ARBITER')
  assert.equal((errors[0].payload as { error: string }).error, 'AI тимчасово не відповів. Можна спробувати ще раз.')
  const thinkingOff = emitted.filter(
    e => e.event === 'room:agent-thinking' && (e.payload as { active: boolean }).active === false
  )
...
test('orchestrator company failure; onAgentRetry resumes company without re-running arbiter first', async () => {
  ...
  const errors = emitted.filter(e => e.event === 'room:agent-error')
  assert.equal(errors.length, 1)
  assert.equal((errors[0].payload as { agentType: string }).agentType, 'AGENT_COMPANY')
  ...
  assert.ok(arbiterCalls > arbiterBeforeRetry, 'after company succeeds, conductor continues to arbiter')
  const agentMessages = emitted.filter(e => e.event === 'room:messages')
  assert.ok(
    agentMessages.some(
      e =>
        (e.payload as { messages: Array<{ authorType: string; content: string }> }).messages[0].authorType ===
          'AGENT_COMPANY' &&
        (e.payload as { messages: Array<{ content: string }> }).messages[0].content === 'Розкажіть про Vue.'
    )
  )
})
...
test('orchestrator onAgentRetry without lastFailedTurn emits thinking false and agent-error', async () => {
  ...
  assert.equal(typeof (agentErrors[0].payload as { error?: string }).error, 'string')
  assert.ok((agentErrors[0].payload as { error: string }).error.trim().length > 0)
})
...
test('orchestrator clears lastFailedTurn on new human message', async () => {
  ...
  orchestrator.onAgentRetry(io, 'interview_1', 'session_1')
  await sleep(40)
  assert.equal(arbiterCalls, callsBeforeRetry, 'onAgentRetry must be no-op after human message cleared lastFailedTurn')
})
...
test('orchestrator stale failure after generation bump must not restore lastFailedTurn', async () => {
  ...
  orchestrator.onAgentRetry(io, 'interview_1', 'session_1')
  await sleep(40)
  assert.equal(arbiterCalls, callsBeforeRetry, 'stale failure must not restore lastFailedTurn for retry')
})
...
test('orchestrator onAgentStop cancels in-flight turn and clears thinking', async () => {
  ...
  const lastThinking = emitted.toReversed().find(e => e.event === 'room:agent-thinking')
