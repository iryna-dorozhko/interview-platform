...
test('HR can access ENDED read-only', () => {
  const result = canAccessInterviewRoom({ hrUserId: 'hr_1', candidateUserId: 'cd_1', status: 'ENDED' }, hrUser)
...
test('candidate can access READY', () => {
  const result = canAccessInterviewRoom({ hrUserId: 'hr_1', candidateUserId: 'cd_1', status: 'READY' }, candidateUser)
  assert.deepEqual(result, { ok: true, readOnly: false })
})

test('HR cannot access another HR interview', () => {
  const result = canAccessInterviewRoom({ hrUserId: 'hr_1', candidateUserId: 'cd_1', status: 'READY' }, otherHr)
  assert.equal(result.ok, false)
  if (!result.ok) assert.equal(result.error, 'Немає доступу')
})

test('candidate cannot access interview linked to another candidate', () => {
  const result = canAccessInterviewRoom({ hrUserId: 'hr_1', candidateUserId: 'cd_2', status: 'READY' }, candidateUser)
  assert.equal(result.ok, false)
  if (!result.ok) assert.equal(result.error, 'Немає доступу')
})
