...
function makeNoopOrchestrator(overrides: Partial<RoomOrchestrator> = {}): RoomOrchestrator {
  return {
    onHumanMessage: () => {},
    onLiveStart: () => {},
    onAgentRetry: () => {},
...
function makeFakePrisma(interview: FakeInterview, sessions: FakeLiveSession[], messages: FakeLiveMessage[]) {
  ...
  return {
    interview: {
      ...
      update: async ({ where, data }: { where: { id: string }; data: { status?: FakeInterview['status'] } }) => {
...
function connectClient(port: number, token: string): Promise<ClientSocket> {
  return new Promise((resolve, reject) => {
    ...
    globalThis.setTimeout(() => reject(new Error('connect timeout')), 5000)
  })
}

function waitForEvent<T>(socket: ClientSocket, event: string, timeoutMs = 5000): Promise<T> {
  return new Promise((resolve, reject) => {
    const timer = globalThis.setTimeout(() => reject(new Error(`Timeout waiting for ${event}`)), timeoutMs)
...
test('room:message persists HUMAN_HR and broadcasts to everyone in the room', async () => {
  ...
  try {
    ...
    hrSocket.emit('room:join', { interviewId: 'interview_1' })
    candidateSocket.emit('room:join', { interviewId: 'interview_1' })
    await Promise.all([waitForEvent(hrSocket, 'room:messages'), waitForEvent(candidateSocket, 'room:messages')])

    const candidateReceived = waitForEvent<{
      messages: Array<{ content: string; authorType: string }>
    }>(candidateSocket, 'room:messages')
...
test('room:join emits room:error for interviews the user cannot access', async () => {
  ...
  try {
    const socket = await connectClient(server.port, signToken({ sub: 'hr_2', email: 'other@test.com', role: 'HR' }))
    socket.emit('room:join', { interviewId: 'interview_1' })
...
test('room:agent-retry calls onAgentRetry for HR in the joined room', async () => {
  ...
  try {
    ...
    const called = await Promise.race([
      retryPromise,
      new Promise<never>((_, reject) => globalThis.setTimeout(() => reject(new Error('Timeout waiting for onAgentRetry')), 5000))
    ])
...
test('room:typing relays to peer without echoing sender', async () => {
  ...
  try {
    ...
    const peerTyping = waitForEvent<{ role: string; isTyping: boolean }>(candidateSocket, 'room:typing')
...
test('room:agent-stop calls onAgentStop for HR in the joined room', async () => {
  ...
  try {
    ...
    const called = await Promise.race([
      stopPromise,
      new Promise<never>((_, reject) => globalThis.setTimeout(() => reject(new Error('Timeout waiting for onAgentStop')), 5000))
    ])
