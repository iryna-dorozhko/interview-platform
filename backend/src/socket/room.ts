...
// Модуль registerRoomHandlers.
export function registerRoomHandlers(io: Server, getPrisma: () => PrismaClient, orchestrator: RoomOrchestrator): void {
  io.use((socket, next) => {
  ...
  io.on('connection', (socket: Socket) => {
    socket.on('room:join', async (payload: RoomJoinPayload) => {
      try {
        ...
        const interviewId = typeof payload?.interviewId === 'string' ? payload.interviewId.trim() : ''
        if (!interviewId) {
