...
// Модуль emitArbiterProcess.
function emitArbiterProcess(io: Server, interviewId: string, command: ParsedArbiterCommand): void {
...
// Модуль emitAgentError.
function emitAgentError(io: Server, interviewId: string, agentType: LiveAuthorType, error: unknown): void {
...
// Модуль applyPendingBeforeRoute.
...
export function createRoomOrchestrator(
  ...
  if (options.runCompanyLiveTurn) {
    ...
    runCompany = (interviewId, sessionId, turnContext) =>
      defaultRunCompanyLiveTurn(getPrisma(), interviewId, sessionId, getLlmProvider(), turnContext)
  } else {
  ...
  if (options.runCandidateLiveTurn) {
    ...
    runCandidate = (interviewId, sessionId, turnContext) =>
      defaultRunCandidateLiveTurn(getPrisma(), interviewId, sessionId, getLlmProvider(), turnContext)
  } else {
      ...
      while (stepsUsed < maxConductorSteps) {
        ...
        if (thisStartAt === 'arbiter') {
          ...
          try {
            ...
            turnFailed = true
            console.error('[orchestrator] AGENT_ARBITER turn failed:', error instanceof Error ? error.message : error)
          ...
          if (command.publicMessage) {
            await saveAndEmit(io, prisma, sessionId, interviewId, 'AGENT_ARBITER', command.publicMessage)
          }
        ...
        const runCompanyActions =
          ...
          command.action === 'CLARIFY' ||
          command.action === 'COMPANY_ANSWER'
        const runCandidateActions = command.action === 'ANSWER' || command.action === 'CANDIDATE_QUESTIONS'
        ...
        if (runCompanyActions && thisStartAt !== 'candidate') {
          ...
          try {
            ...
            if (reply.post && reply.message) {
              await saveAndEmit(io, prisma, sessionId, interviewId, 'AGENT_COMPANY', reply.message)
            ...
            turnFailed = true
            console.error('[orchestrator] AGENT_COMPANY turn failed:', error instanceof Error ? error.message : error)
            if (state.generation === capturedGeneration) {
        ...
        if (runCandidateActions) {
          ...
          try {
            ...
            if (reply.post && reply.message) {
              const prismaConfidence = reply.confidence == null ? null : toPrismaCandidateConfidence(reply.confidence)
              await saveAndEmit(io, prisma, sessionId, interviewId, 'AGENT_CANDIDATE', reply.message, prismaConfidence)
            ...
            turnFailed = true
            console.error('[orchestrator] AGENT_CANDIDATE turn failed:', error instanceof Error ? error.message : error)
            if (state.generation === capturedGeneration) {
