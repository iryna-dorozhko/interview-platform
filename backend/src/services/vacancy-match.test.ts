...
test('pickNextOffer returns null when all rejected', () => {
  const next = pickNextOffer([{ vacancyId: 'a', title: 'A', matchScore: 50 }], new Set(['a']))
...
test('pickTopOffers returns empty array when all rejected', () => {
  const top = pickTopOffers([{ vacancyId: 'v1', title: 'One', matchScore: 50 }], new Set(['v1']))
...
test('ensureMatchScores returns empty without calling LLM when no confirmed vacancies', async () => {
  ...
  const offers = await ensureMatchScores(fakePrisma as unknown as PrismaClient, fakeLlm, 'cd_1')
...
test('getTopMatchOffers skips rejected vacancies and returns remaining', async () => {
  ...
  const offers = await getTopMatchOffers(fakePrisma as unknown as PrismaClient, fakeLlm, 'cd_1')
