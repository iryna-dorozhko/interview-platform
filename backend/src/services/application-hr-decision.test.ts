...
import { applicationStatusFromDecisionType, applyTerminalApplicationStatus } from './application-hr-decision'
...
test('applicationStatusFromDecisionType maps all decision types', () => {
  ...
  assert.equal(applicationStatusFromDecisionType('ADDITIONAL_MEETING'), 'ADDITIONAL_MEETING')
})
