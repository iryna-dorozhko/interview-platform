...
test('formatSalaryDisplay returns displayText when present', () => {
  assert.equal(formatSalaryDisplay({ displayText: '$3000–4500 gross, USD' }), '$3000–4500 gross, USD')
})
...
test('formatWorkFormatDisplay strips Формат prefix', () => {
  assert.equal(formatWorkFormatDisplay(['Формат: remote, 2 дні в офісі']), 'remote, 2 дні в офісі')
})
