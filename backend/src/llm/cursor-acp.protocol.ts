...
export class NdjsonDecoder {
  ...
  push(chunk: Buffer): string[] {
    let data = this.buffered.length === 0 ? chunk : Buffer.concat([this.buffered, chunk])
    ...
    while (data.length > 0) {
      ...
      if (newlineIndex === -1) {
        if (data.length > this.maxLineBytes) {
          throw new AcpProtocolError(`ACP NDJSON line exceeds ${this.maxLineBytes} bytes`)
        }
...
export function parseJsonRpcMessage(line: string): JsonRpcMessage {
  ...
  if (!hasId || !isJsonRpcId(value.id) || hasResult === hasError) {
...
function parseAuthMethods(value: unknown): AcpInitializeResult['authMethods'] {
  ...
  return value.map(item => {
    ...
    const parsed = {
      ...
      description:
        method.description === undefined ? undefined : requireNonEmptyString(method.description, 'initialize result')
    }
    return parsed
  })
}

// Парсить InitializeResult.
export function parseInitializeResult(value: unknown): AcpInitializeResult {
  ...
  const agentCapabilities = requireRecord(result.agentCapabilities, 'initialize result')
  if (agentCapabilities.sessionCapabilities !== undefined && !isRecord(agentCapabilities.sessionCapabilities)) {
    throw new AcpProtocolError('invalid initialize result')
  }
...
// Парсить ConfigOptionValues.
function parseConfigOptionValues(value: unknown): AcpConfigOptionValue[] | undefined {
  if (value === undefined) return undefined
  if (!Array.isArray(value)) {
  ...
  return value.map(item => {
    ...
    return {
      ...
      description:
        option.description === undefined ? undefined : requireNonEmptyString(option.description, 'session/new result')
    }
  })
}

// Парсить ConfigOptions.
  ...
  return value.map(item => {
    ...
    return {
      ...
      description:
        option.description === undefined ? undefined : requireNonEmptyString(option.description, 'session/new result'),
      category:
        option.category === undefined ? undefined : requireNonEmptyString(option.category, 'session/new result'),
...
function parseModes(value: unknown): AcpSessionNewResult['modes'] {
  ...
  return {
    currentModeId: requireNonEmptyString(modes.currentModeId, 'session/new result'),
    availableModes: modes.availableModes.map(item => {
      ...
      return {
        ...
        description:
          mode.description === undefined ? undefined : requireNonEmptyString(mode.description, 'session/new result')
      }
    })
  }
}

// Парсить SessionNewResult.
...
export function parseSessionUpdate(value: unknown): AcpSessionUpdate {
  ...
  const sessionUpdate = requireNonEmptyString(update.sessionUpdate, 'session/update params')
  ...
  return {
    sessionId: requireNonEmptyString(params.sessionId, 'session/update params'),
...
// Модуль selectPermissionRejection.
export function selectPermissionRejection(
  options: AcpPermissionOption[]
): { outcome: { outcome: 'selected'; optionId: string } } | { outcome: { outcome: 'cancelled' } } {
  const rejected =
    options.find(option => option.kind === 'reject_always') ?? options.find(option => option.kind === 'reject_once')
