...
import { isRetryableLlmError, toSafeLlmErrorMessage, withLlmRetry, SAFE_LLM_ERROR_UK } from './retry'
...
test('isRetryableLlmError rejects LLM timeout errors', () => {
  assert.equal(isRetryableLlmError(new LlmUnavailableError('Cursor ACP prompt timed out after 120000ms')), false)
  assert.equal(isRetryableLlmError(new LlmUnavailableError('Cursor ACP session/new timed out after 30000ms')), false)
  assert.equal(isRetryableLlmError(new LlmUnavailableError('Cursor ACP is unavailable: down')), true)
})

test('toSafeLlmErrorMessage keeps rate-limit Ukrainian text', () => {
  const err = new LlmUnavailableError('LLM API: перевищено ліміт запитів. Змініть LLM_PROVIDER у .env або зачекайте.')
