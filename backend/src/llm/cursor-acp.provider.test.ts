...
import type { CursorAcpClientLike } from './cursor-acp.provider'
import { LlmEmptyResponseError, LlmUnavailableError } from './errors'

function makeConfig() {
  return readCursorAcpConfig({ CURSOR_ACP_CWD: '/tmp/cursor-acp-provider-test' }, { tmpdir: () => '/tmp' })
}

function makeClient(
  completePrompt: CursorAcpClientLike['completePrompt'],
  close: CursorAcpClientLike['close'] = async () => {}
): CursorAcpClientLike {
...
test('provider rejects empty output', async () => {
  ...
  await assert.rejects(provider.complete([{ role: 'user', content: 'hello' }]), LlmEmptyResponseError)
})
...
test('provider maps missing executable and expired auth to actionable errors', async () => {
  const missing = createCursorAcpProvider(makeConfig(), {
    client: makeClient(async () => {
      throw new AcpClientError('spawn', 'Failed to start Cursor ACP process', {
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof LlmUnavailableError && /executable.*not found/i.test(error.message)
  )

  const auth = createCursorAcpProvider(makeConfig(), {
    client: makeClient(async () => {
      throw new AcpClientError('authentication', 'Cursor ACP request failed: authentication required')
    })
  })
  await assert.rejects(
    auth.complete([{ role: 'user', content: 'hello' }]),
    (error: unknown) => error instanceof LlmUnavailableError && /agent login/.test(error.message)
  )
})
