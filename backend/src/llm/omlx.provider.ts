    ...
    async complete(messages: ChatMessage[], options?: LlmCompleteOptions): Promise<string> {
      ...
      try {
        ...
        if (causeCode === 'ECONNREFUSED' || detail.includes('ECONNREFUSED')) {
          throw new LlmUnavailableError(`omlx server not reachable at ${baseUrl}`)
        }
      ...
      if (!response.ok) {
        ...
        throw new LlmUnavailableError(`omlx request failed (${response.status}): ${body}`)
      }
