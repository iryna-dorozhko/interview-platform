...
const ACP_AUTH_ERROR_RE = /auth|login|credential|unauthori[sz]ed|token/i
...
const defaultSpawn: SpawnAcp = (executable, args, options) => spawnChild(executable, args, options) as AcpChild

// Модуль defaultPrepareRuntime.
...
export class CursorAcpClient {
  ...
  private readonly onStdoutEnd = (): void => {
    ...
    if (this.state !== 'stopping' && this.state !== 'closed') {
      this.failTransport(new AcpClientError('transport', 'Cursor ACP stdout closed unexpectedly'))
    }
  }
  ...
  private readonly onChildError = (error: Error): void => {
    this.failTransport(new AcpClientError('spawn', 'Cursor ACP process error', { cause: error }))
  }
  ...
  async completePrompt(prompt: string): Promise<string> {
    ...
    if (this.sessions.has(created.sessionId)) {
      throw new AcpClientError('protocol', `Cursor ACP returned duplicate active sessionId: ${created.sessionId}`)
    }
    ...
    try {
      ...
      this.armPromptIdleWatchdog(
        created.sessionId,
        new AcpClientError('timeout', `Cursor ACP prompt idle timed out after ${this.config.promptIdleTimeoutMs}ms`)
      )
      ...
      if (result.stopReason === 'error') {
        throw new AcpClientError('protocol', 'Cursor ACP prompt terminated with error')
      }
  ...
  private async ensureReady(): Promise<void> {
    ...
    this.startPromise = this.withTimeout(this.start(), this.config.startupTimeoutMs, timeoutError, () =>
  ...
  private async start(): Promise<void> {
    ...
    try {
      ...
      throw new AcpClientError('spawn', 'Failed to start Cursor ACP process', { cause: error })
    }
    ...
    if (!initialized.authMethods?.some(method => method.id === 'cursor_login')) {
      throw new AcpClientError('authentication', 'Cursor ACP did not advertise cursor_login authentication')
    }
    ...
    const effectiveTimeout = timeout ?? {
      ...
      onTimeout: () =>
        this.abortCurrentProcess(
          new AcpClientError('timeout', `Cursor ACP ${method} timed out after ${this.config.requestTimeoutMs}ms`)
        )
    }
  ...
  private write(message: unknown): Promise<void> {
    ...
    if (!child || child.stdin.destroyed || child.stdin.writableEnded) {
      return Promise.reject(new AcpClientError('transport', 'Cursor ACP stdin is unavailable'))
    }
  ...
  private handleMessage(message: JsonRpcMessage): void {
    ...
    if (message.error) {
      const kind =
        message.error.code === -32000 && ACP_AUTH_ERROR_RE.test(message.error.message)
          ? 'authentication'
          : 'protocol'
      pending.reject(new AcpClientError(kind, `Cursor ACP request failed: ${message.error.message}`))
    } else {
  ...
  private handleSessionUpdate(params: unknown): void {
    ...
    if (session?.collecting && update.sessionUpdate === 'agent_message_chunk' && update.content?.type === 'text') {
  ...
  private async confirmAskMode(created: AcpSessionNewResult): Promise<void> {
    const modeOption = created.configOptions?.find(option => option.category === 'mode')
    if (modeOption?.options?.some(option => option.value === 'ask')) {
      ...
      if (!this.configResultConfirmsAsk(result)) {
        throw new AcpClientError('protocol', 'Cursor ACP did not confirm ask mode')
      }
      return
    }

    const askMode = created.modes?.availableModes.find(mode => mode.id === 'ask')
    if (!askMode) {
      throw new AcpClientError('protocol', 'Cursor ACP session does not support ask mode')
    }
  ...
  private configResultConfirmsAsk(value: unknown): boolean {
    ...
    return value.configOptions.some(
      option => isRecord(option) && option.category === 'mode' && option.currentValue === 'ask'
    )
  }
  ...
  private cleanupSession(sessionId: string, cancel: boolean): Promise<void> {
    const session = this.sessions.get(sessionId)
    ...
    session.cleanupPromise = (async () => {
      try {
        ...
        this.abortCurrentProcess(error instanceof Error ? error : new AcpClientError('transport', String(error)))
      }
    })().finally(() => {
  ...
  private failTransport(error: unknown, terminateChild = true): void {
    const normalized = error instanceof Error ? error : new AcpClientError('transport', String(error))
    ...
    this.stderrTail = ''
    this.completedSessionCount = 0
    if (!this.recyclePromise && this.state !== 'stopping' && this.state !== 'closed') {
      this.state = 'idle'
    }
  }
  ...
  private async performShutdown(): Promise<void> {
    ...
    const cleanup = Promise.allSettled(
      Array.from(this.sessions.keys(), sessionId => this.cleanupSession(sessionId, true))
    )
    await this.waitWithin(cleanup, this.config.shutdownGraceMs)

    const closedError = new AcpClientError('closed', 'Cursor ACP provider is closed')
    for (const pending of this.pending.values()) {
      if (pending.timer) clearTimeout(pending.timer)
  ...
  private waitForDetachedExit(child: AcpChild, timeoutMs: number): Promise<boolean> {
    return new Promise(resolve => {
      const onExit = (): void => {
        clearTimeout(timer)
        child.off('exit', onExit)
