...
export const SAFE_LLM_ERROR_UK = 'AI тимчасово не відповів. Можна спробувати ще раз.'
...
// Модуль withLlmRetry.
export async function withLlmRetry<T>(fn: () => Promise<T>, options: WithLlmRetryOptions = {}): Promise<T> {
  ...
  if (isGeminiRateLimitError(lastError)) {
    throw new LlmUnavailableError('LLM API: перевищено ліміт запитів. Змініть LLM_PROVIDER у .env або зачекайте.')
  }

  console.error(`[llm-retry:${label}] exhausted:`, lastError instanceof Error ? lastError.message : lastError)
  throw lastError
}
