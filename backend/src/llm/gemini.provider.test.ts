...
test('buildGeminiHistory prepends synthetic user turn when history starts with agent message', () => {
  const history = buildGeminiHistory([{ role: 'assistant', content: 'Вітаю! Розкажіть про вакансію.' }])
...
test('parseGeminiRetryDelayMs reads retry hint from 429 error', () => {
  const delay = parseGeminiRetryDelayMs(new Error('[429 Too Many Requests] Please retry in 9.743665662s'))
