...
import { LlmEmptyResponseError, LlmError, LlmUnavailableError } from './errors'
...
function mapCursorAcpError(error: unknown): LlmUnavailableError {
  ...
  if (error instanceof AcpClientError) {
    if (error.kind === 'authentication') {
      return new LlmUnavailableError('Cursor authentication is missing or expired. Run `agent login` and try again.')
    }
    if (error.kind === 'initialization') {
      return new LlmUnavailableError(`Cursor ACP initialization failed: ${error.message}`)
    }
    if (error.kind === 'protocol') {
      return new LlmUnavailableError(`Cursor ACP protocol error: ${error.message}`)
    }
    ...
    return new LlmUnavailableError(`Cursor ACP is unavailable: ${error.message}`)
  }
