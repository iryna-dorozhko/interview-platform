...
export function readCursorAcpConfig(
  ...
  const cwd = env.CURSOR_ACP_CWD ?? path.join(runtime.tmpdir(), 'interview-platform-cursor-acp')
  ...
  return {
    executable,
    cwd,
    startupTimeoutMs: readPositiveInteger('CURSOR_ACP_STARTUP_TIMEOUT_MS', env.CURSOR_ACP_STARTUP_TIMEOUT_MS, 15_000),
    promptTimeoutMs: readPositiveInteger('CURSOR_ACP_PROMPT_TIMEOUT_MS', env.CURSOR_ACP_PROMPT_TIMEOUT_MS, 120_000),
    promptIdleTimeoutMs: readPositiveInteger(
      'CURSOR_ACP_PROMPT_IDLE_TIMEOUT_MS',
      env.CURSOR_ACP_PROMPT_IDLE_TIMEOUT_MS,
      45_000
    ),
    requestTimeoutMs: readPositiveInteger('CURSOR_ACP_REQUEST_TIMEOUT_MS', env.CURSOR_ACP_REQUEST_TIMEOUT_MS, 30_000),
    shutdownGraceMs: readPositiveInteger('CURSOR_ACP_SHUTDOWN_GRACE_MS', env.CURSOR_ACP_SHUTDOWN_GRACE_MS, 5_000),
    terminateGraceMs: readPositiveInteger('CURSOR_ACP_TERMINATE_GRACE_MS', env.CURSOR_ACP_TERMINATE_GRACE_MS, 2_000),
    maxSessions: readPositiveInteger('CURSOR_ACP_MAX_SESSIONS', env.CURSOR_ACP_MAX_SESSIONS, 100),
    maxLineBytes: readPositiveInteger('CURSOR_ACP_MAX_LINE_BYTES', env.CURSOR_ACP_MAX_LINE_BYTES, 1_048_576),
    childEnv: buildChildEnv(env)
  }
}

// Перевіряє MissingFile.
...
export async function assertNoConfiguredMcp(
  ...
  const configPaths = [
    ...
    config.childEnv.HOME ? path.join(config.childEnv.HOME, '.cursor', 'mcp.json') : null
  ]
