...
import { AcpClientError, CursorAcpClient } from './cursor-acp.client'
import type { AcpChild, SpawnAcp } from './cursor-acp.client'
...
class FakeAcpProcess extends EventEmitter implements AcpChild {
  ...
  constructor(private readonly onMessage: (message: JsonObject, process: FakeAcpProcess) => void) {
    super()
...
function makeHarness(options: HarnessOptions = {}) {
  ...
  const spawn: SpawnAcp = () => {
    ...
    const process = new FakeAcpProcess((message, current) => {
      ...
      if (message.method === 'session/new') {
        ...
        nextSession += 1
        const sessionId = options.duplicateSessionId ? 'duplicate' : `session-${nextSession}`
        current.send({
          jsonrpc: '2.0',
          id,
      ...
      if (message.method === 'session/prompt') {
        ...
        if (options.interleavePrompts) {
          ...
          if (pendingPrompts.length === 2) {
            ...
            const firstSession = (first.request.params as JsonObject).sessionId as string
            const secondSession = (second.request.params as JsonObject).sessionId as string
  ...
  return {
    ...
    methods(method: string): JsonObject[] {
      return processes.flatMap(process => process.messages.filter(message => message.method === method))
    },
    responses(id: number): JsonObject[] {
      return processes.flatMap(process =>
        process.messages.filter(message => message.id === id && typeof message.method !== 'string')
      )
    },
...
function sendChunk(process: FakeAcpProcess, sessionId: string, text: string): void {
  process.send({
    jsonrpc: '2.0',
    method: 'session/update',
    params: {
...
function makeClient(harness: ReturnType<typeof makeHarness>): CursorAcpClient {
...
test('multiple completions share one process initialization', async () => {
  ...
  const [first, second] = await Promise.all([client.completePrompt('first'), client.completePrompt('second')])
  ...
  const capabilities = (harness.methods('initialize')[0].params as JsonObject).clientCapabilities
...
test('parallel sessions route text chunks without mixing thought or tool updates', async () => {
  ...
  assert.deepEqual(await Promise.all([client.completePrompt('A'), client.completePrompt('B')]), ['A1A2', 'B1B2'])
  const sessionIds = harness.methods('session/prompt').map(request => (request.params as JsonObject).sessionId)
...
test('client classifies generic -32000 errors as protocol errors, not auth', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof AcpClientError && error.kind === 'protocol'
  )
  await client.close()
})
...
test('client rejects duplicate active session IDs', async () => {
  ...
  const results = await Promise.allSettled([client.completePrompt('A'), client.completePrompt('B')])
...
test('startup timeout terminates one uninitialized process and rejects all waiters', async () => {
  ...
  const client = new CursorAcpClient(makeConfig({ startupTimeoutMs: 15 }), {
    spawn: harness.spawn,
    prepareRuntime: async () => {}
  })
  ...
  assert.ok(results.every(result => result.status === 'rejected' && /startup timed out/.test(String(result.reason))))
...
test('hung session/close after successful prompt still resolves and allows restart', async () => {
  ...
  const text = await Promise.race([
    ...
    new Promise<string>((_, reject) => globalThis.setTimeout(() => reject(new Error('completePrompt hung past deadline')), 500))
  ])
...
test('hung session/new times out without blocking forever', async () => {
  ...
  await assert.rejects(
    Promise.race([
      ...
      new Promise((_, reject) => globalThis.setTimeout(() => reject(new Error('session/new hang past deadline')), 500))
    ]),
    /timed out/
  )
  const elapsed = Date.now() - started
...
test('prompt timeout still settles when session/close also hangs', async () => {
  ...
  await assert.rejects(
    Promise.race([
      ...
      new Promise((_, reject) => globalThis.setTimeout(() => reject(new Error('prompt+close hang past deadline')), 500))
    ]),
    /prompt timed out/
  )
  const elapsed = Date.now() - started
...
test('prompt idle timeout fires when no chunks arrive before hard prompt timeout', async () => {
  ...
  await assert.rejects(
    Promise.race([
      ...
      new Promise((_, reject) => globalThis.setTimeout(() => reject(new Error('idle hang past deadline')), 1_000))
    ]),
...
test('prompt timeout cancels and capability-closes only its session', async () => {
  ...
  const supportedClient = new CursorAcpClient(makeConfig({ promptTimeoutMs: 15 }), {
    spawn: supported.spawn,
    prepareRuntime: async () => {}
  })

  await assert.rejects(supportedClient.completePrompt('hang'), /prompt timed out/)
  ...
  const unsupportedClient = new CursorAcpClient(makeConfig({ promptTimeoutMs: 15 }), {
    spawn: unsupported.spawn,
    prepareRuntime: async () => {}
  })
  await assert.rejects(unsupportedClient.completePrompt('hang'), /prompt timed out/)
...
test('child crash rejects all active completions and the next call lazily restarts', async () => {
  const harness = makeHarness({ hangPrompt: true })
  const client = makeClient(harness)
  const firstWave = Promise.allSettled([client.completePrompt('A'), client.completePrompt('B')])
...
test('malformed protocol rejects every active completion and permits restart', async () => {
  const harness = makeHarness({ hangPrompt: true })
  const client = makeClient(harness)
  const active = Promise.allSettled([client.completePrompt('A'), client.completePrompt('B')])
  await harness.waitForMethods('session/prompt', 2)
...
test('fatal transport corruption escalates termination for a stubborn child', async () => {
  ...
  const client = new CursorAcpClient(makeConfig({ terminateGraceMs: 5 }), {
    spawn: harness.spawn,
    prepareRuntime: async () => {}
  })
...
test('missing close capability recycles only after the session limit becomes idle', async () => {
  ...
  const client = new CursorAcpClient(makeConfig({ maxSessions: 2 }), {
    spawn: harness.spawn,
    prepareRuntime: async () => {}
  })

  await Promise.all([client.completePrompt('A'), client.completePrompt('B')])
  assert.equal(harness.spawnCount, 1)
