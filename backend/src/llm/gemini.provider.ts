...
const QUOTA_EXCEEDED_RE = /quota exceeded/i
const GEMINI_RETRY_DELAY_RE = /Please retry in (\d+(?:\.\d+)?)s/i

// Перевіряє GeminiRateLimitError.
export function isGeminiRateLimitError(error: unknown): boolean {
  const msg = error instanceof Error ? error.message : String(error)
  return msg.includes('429') || msg.includes('Too Many Requests') || QUOTA_EXCEEDED_RE.test(msg)
}

// Парсить GeminiRetryDelayMs.
export function parseGeminiRetryDelayMs(error: unknown): number {
  const msg = error instanceof Error ? error.message : String(error)
  const match = msg.match(GEMINI_RETRY_DELAY_RE)
  if (match) {
...
// Gemini requires chat history passed to startChat() to begin with a "user" turn. Our agents can legitimately speak first (e.g. greeting before any user message exists), which would otherwise start the history with "model" — prepend a synthetic user turn so the request is well-formed.
...
export function mapGeminiCompleteError(error: unknown): Error {
  ...
  return new LlmUnavailableError(detail ? `Gemini API unavailable: ${detail}` : 'Gemini API unavailable')
}

// Створює GeminiProvider.
