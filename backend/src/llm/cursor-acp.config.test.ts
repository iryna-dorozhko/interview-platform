...
import { assertNoConfiguredMcp, readCursorAcpConfig } from './cursor-acp.config'
...
test('readCursorAcpConfig rejects invalid numeric values', () => {
  ...
  for (const [name, value] of cases) {
    assert.throws(() => readCursorAcpConfig({ [name]: value }), new RegExp(`${name} must be a positive integer`))
  }
})

test('readCursorAcpConfig rejects empty executable and relative cwd', () => {
  assert.throws(() => readCursorAcpConfig({ CURSOR_ACP_EXECUTABLE: '  ' }), /CURSOR_ACP_EXECUTABLE must not be empty/)
  assert.throws(() => readCursorAcpConfig({ CURSOR_ACP_CWD: 'relative/path' }), /CURSOR_ACP_CWD must be absolute/)
})
...
test('assertNoConfiguredMcp rejects configured project or user MCP servers', async () => {
  for (const configuredPath of ['/tmp/interview-platform-cursor-acp/.cursor/mcp.json', '/home/test/.cursor/mcp.json']) {
...
test('assertNoConfiguredMcp rejects malformed or invalid MCP config', async () => {
  const invalidContents = ['{', JSON.stringify({ mcpServers: [] }), JSON.stringify({ mcpServers: null })]
