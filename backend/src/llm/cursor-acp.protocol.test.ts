...
test('NdjsonDecoder rejects invalid UTF-8 and unterminated final data', () => {
  ...
  assert.throws(() => invalidUtf8.push(Buffer.from([0xC3, 0x28, 0x0A])), /invalid UTF-8/)
...
test('parseJsonRpcMessage parses requests, notifications, and responses', () => {
  assert.deepEqual(parseJsonRpcMessage('{"jsonrpc":"2.0","id":0,"method":"initialize","params":{}}'), {
    jsonrpc: '2.0',
    id: 0,
    method: 'initialize',
    params: {}
  })
  assert.deepEqual(parseJsonRpcMessage('{"jsonrpc":"2.0","method":"session/update"}'), {
    jsonrpc: '2.0',
    method: 'session/update'
  })
  assert.deepEqual(parseJsonRpcMessage('{"jsonrpc":"2.0","id":"a","result":null}'), {
    jsonrpc: '2.0',
    id: 'a',
    result: null
  })
...
test('ACP result validators reject missing or invalid required values', () => {
  assert.throws(() => parseInitializeResult({ protocolVersion: '1', agentCapabilities: {} }), /initialize result/)
  assert.throws(() => parseSessionNewResult({ sessionId: '' }), /session\/new result/)
  assert.throws(() => parsePromptResult({ stopReason: 1 }), /session\/prompt result/)
})
