...
test('arbiter prompt waits only after unknown confidence deferral', () => {
  ...
  assert.doesNotMatch(ARBITER_AGENT_SYSTEM_PROMPT_UK, /припущення.*підтверд.*WAIT/s)
})

test('arbiter prompt routes human-to-candidate-agent address as ANSWER', () => {
  assert.match(ARBITER_AGENT_SYSTEM_PROMPT_UK, /HUMAN_CANDIDATE|людин.*кандидат|кандидат.*людин/i)
  assert.match(ARBITER_AGENT_SYSTEM_PROMPT_UK, /зверт.*(Candidate|агент)|агент.*кандидат.*ANSWER|ANSWER.*агент/i)
...
test('parseArbiterCommand parses WAIT', () => {
  const result = parseArbiterCommand('{ "action": "WAIT", "summaryUk": "Розмова йде природно." }')
...
test('parseArbiterCommand throws when summaryUk is empty', () => {
  assert.throws(() => parseArbiterCommand('{ "action": "WAIT", "summaryUk": "   " }'), ArbiterReplyParseError)
})

test('parseArbiterCommand throws when START lacks publicMessage', () => {
  assert.throws(() => parseArbiterCommand('{ "action": "START", "summaryUk": "Старт" }'), ArbiterReplyParseError)
})

test('parseArbiterCommand throws on unknown action', () => {
  assert.throws(() => parseArbiterCommand('{ "action": "NOPE", "summaryUk": "x" }'), ArbiterReplyParseError)
})
