...
import { extractVacancyOffer, buildDecisionLetterMessages, normalizeDecisionLetter } from './decision-letter-agent'
...
test('extractVacancyOffer returns empty when all unspecified', () => {
  const result = extractVacancyOffer({
    ...
    workConditions: ['Формат: не вказано', 'Графік: не вказано']
  })
