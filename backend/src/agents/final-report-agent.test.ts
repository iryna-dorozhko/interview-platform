...
test('parseFinalReport forces HIRE when all critical are met even if LLM returned MAYBE', () => {
  const result = parseFinalReport(sampleReportJson({ recommendation: 'MAYBE' }), sampleRequirements)
  assert.equal(result.recommendation, 'HIRE')
})
...
test('parseFinalReport forces HIRE when desired unmet but all critical met', () => {
  const result = parseFinalReport(sampleReportJson({ recommendation: 'REJECT' }), sampleRequirements)
  assert.equal(result.recommendation, 'HIRE')
})
...
test('FINAL_REPORT_SYSTEM_PROMPT_UK requires assessments and contextFit without matchScore', () => {
  ...
  assert.match(FINAL_REPORT_SYSTEM_PROMPT_UK, /всі critical мають status met.*МАЄ бути HIRE|МАЄ бути HIRE/i)
  assert.match(FINAL_REPORT_SYSTEM_PROMPT_UK, /unmet або unknown — recommendation НЕ може бути HIRE/i)
  ...
  assert.match(FINAL_REPORT_SYSTEM_PROMPT_UK, /відхилитись від цього правила ЛИШЕ|можна відхилитись/i)
  assert.doesNotMatch(
