...
import { buildCandidateAgentMessages, parseCandidateProfileExtraction } from './candidate-agent'
...
test('candidate system prompt asks for concrete projects with bounded follow-ups', () => {
  ...
  assert.doesNotMatch(CANDIDATE_AGENT_SYSTEM_PROMPT_UK, /Кількість уточнюючих раундів на тему не обмежена/)
})
...
test('candidate system prompt READY gate requires all five profile topics', () => {
  ...
  assert.doesNotMatch(CANDIDATE_AGENT_SYSTEM_PROMPT_UK, /одне коротке уточнювальне питання/i)
})
