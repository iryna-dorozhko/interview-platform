...
// Форматує FollowUpContext.
export function formatFollowUpContext(interview?: CompanyLiveInterviewContext | null): string {
  if (!interview || interview.kind !== 'ADDITIONAL_MEETING' || !interview.followUpFromFinalReport) {
    return 'none'
  }
...
function buildSystemPrompt(
  ...
  return COMPANY_LIVE_AGENT_SYSTEM_PROMPT_UK.replace('{{COMPANY_PROFILE}}', formatProfileBlock(companyProfile)).replace(
    '{{FOLLOW_UP_CONTEXT}}',
    formatFollowUpContext(interview)
  )
}
