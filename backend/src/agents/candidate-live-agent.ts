...
const CONFIDENCE_LEVELS = new Set<CandidateConfidenceLevel>(['confirmed', 'inferred', 'unknown'])

// Парсить ConfidenceLevel.
...
// Модуль toPrismaCandidateConfidence.
...
function buildSystemPrompt(candidateProfile: CandidateLiveProfileContext): string {
  return CANDIDATE_LIVE_AGENT_SYSTEM_PROMPT_UK.replace('{{CANDIDATE_PROFILE}}', formatProfileBlock(candidateProfile))
}
...
const CANDIDATE_AUTHOR_TYPES = new Set<LiveAuthorType>(['AGENT_CANDIDATE', 'HUMAN_CANDIDATE'])

// Модуль collectRecentInterviewerQuestions.
...
// Модуль collectOpenInterviewerQuestions.
...
// Форматує InterviewerQuestionsBlock.
...
// Форматує OpenInterviewerQuestionsBlock.
...
// Форматує CandidateTurnNudge.
export function formatCandidateTurnNudge(turnContext: LiveAgentTurnContext, history: LiveHistoryItem[] = []): string {
...
export async function runCandidateLiveTurn(
  ...
  const requireConfidence = turnContext?.action === 'ANSWER' || turnContext?.action === undefined
