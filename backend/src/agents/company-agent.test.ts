...
import { buildCompanyAgentMessages, buildProfileExtractionMessages, parseVacancyProfileExtraction } from './company-agent'
import type { CompanyAgentHrProfileContext } from './company-agent'
...
test('company agent system prompt asks role first and seniority only neutrally if missing', () => {
  assert.doesNotMatch(COMPANY_AGENT_SYSTEM_PROMPT_UK, /Посада \(роль, рівень — junior\/middle\/senior, команда\)/)
  assert.match(
    COMPANY_AGENT_SYSTEM_PROMPT_UK,
    /перше питання лише про (назву )?посади|назву посади\/роль|лише про назву посади/i
  )
  assert.match(COMPANY_AGENT_SYSTEM_PROMPT_UK, /досвід|грейд|рівень відповідальності/i)
  assert.match(COMPANY_AGENT_SYSTEM_PROMPT_UK, /не нав.?язуй|не починай.*junior|junior\/middle\/senior/i)
...
test('buildProfileExtractionMessages prepends extraction system prompt and joins transcript as one user message', () => {
  ...
  assert.equal(messages[1].content, 'HR: Middle Backend Developer\nАгент: Які вимоги?')
})
