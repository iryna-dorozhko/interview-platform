...
import { CANDIDATE_SUMMARY_SYSTEM_PROMPT_UK, VACANCY_MATCH_SYSTEM_PROMPT_UK } from './prompts/vacancy-match.uk'
...
// Парсить VacancyMatchAssessments.
  ...
  for (const vacancy of vacancies) {
    ...
    for (const assessment of result.assessments) {
      ...
      if (assessment.priority !== expectedPriority) {
        throw new VacancyMatchExtractionError(`priority mismatch for requirement: ${assessment.requirement}`)
      }
    ...
    if (seen.size !== expected.size) {
      throw new VacancyMatchExtractionError(`incomplete assessments for vacancyId: ${vacancy.vacancyId}`)
    }
...
// Будує CandidateSummaryMessages.
