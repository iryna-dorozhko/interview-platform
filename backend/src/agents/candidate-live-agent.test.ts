...
test('parseCandidateLiveReply maps confidence to needsHuman', () => {
  ...
  const inferred = parseCandidateLiveReply('{ "post": true, "message": "З анкети видно…", "confidence": "inferred" }', {
    requireConfidence: true
  })
...
test('buildCandidateLiveMessages includes candidate profile and HR/Company prefixes', () => {
  ...
  assert.ok(
    messages[0].content.includes(CANDIDATE_LIVE_AGENT_SYSTEM_PROMPT_UK.split('{{CANDIDATE_PROFILE}}')[0].trimEnd())
  )
...
test('buildCandidateLiveMessages appends nudge when last message is from Company Agent', () => {
  ...
  assert.match(
    ...
    new RegExp(COMPANY_QUESTION_NUDGE_UK.replaceAll(/[.*+?^${}()|[\]\\]/g, String.raw`\$&`))
  )
...
test('buildCandidateLiveMessages uses turnContext ANSWER nudge', () => {
  ...
  assert.match(messages.at(-1)!.content, new RegExp(ANSWER_NUDGE_UK.replaceAll(/[.*+?^${}()|[\]\\]/g, String.raw`\$&`)))
})
...
test('candidate live prompt requires third person', () => {
  ...
  assert.doesNotMatch(CANDIDATE_LIVE_AGENT_SYSTEM_PROMPT_UK, /Відповідай від імені кандидата \(перша особа/)
  assert.doesNotMatch(
...
test('collectOpenInterviewerQuestions returns all interviewer messages when candidate never spoke', () => {
  ...
  assert.deepEqual(collectOpenInterviewerQuestions(history), ['Перше питання.', 'Уточнення від HR.'])
})
...
test('runCandidateLiveTurn loads profile, calls LLM, parses reply', async () => {
  const prisma = {
    ...
    liveMessage: {
      findMany: async () => [{ authorType: 'HUMAN_HR', content: 'Який ваш досвід з Node.js?' }]
    }
