...
export type RecommendationOverrideKind = 'culture_fit' | 'soft_skills' | 'critical_gap_ok' | 'red_flag' | 'other'
...
// Модуль validateAssessments.
function validateAssessments(assessments: unknown, requirements: VacancyRequirements): RequirementAssessment[] {
  ...
  for (const item of assessments) {
    ...
    if (assessment.priority !== expectedPriority) {
      throw new FinalReportExtractionError(`priority mismatch for requirement: ${assessment.requirement}`)
    }
...
// Форматує LiveTranscript.
export function formatLiveTranscript(messages: LiveTranscriptItem[]): string {
  if (messages.length === 0) return '(розмова порожня)'
  return messages.map(item => `[${formatAuthorLabel(item)}] ${item.content}`).join('\n')
}

// Парсить FinalReport з JSON-відповіді LLM.
export function parseFinalReport(rawText: string, requirements: VacancyRequirements): ExtractedFinalReport {
