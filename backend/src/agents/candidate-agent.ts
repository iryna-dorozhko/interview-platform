...
export function extractFirstName(fullName: string | null | undefined): string | null {
  ...
  const [firstName] = trimmed.split(WHITESPACE_SPLIT_RE)
  return firstName || null
}

// Будує CandidateAgentSystemPrompt.
...
// Витягує ContactPreviewFromHistory.
...
// Парсить CandidateProfileExtraction з JSON-відповіді LLM.
...
// Будує CandidateProfileExtractionMessages.
export function buildCandidateProfileExtractionMessages(history: CandidatePrepHistoryItem[]): ChatMessage[] {
  const transcript = history
    .map(item => `${item.authorType === 'HUMAN_CANDIDATE' ? 'Кандидат' : 'Агент'}: ${item.content}`)
    .join('\n')
