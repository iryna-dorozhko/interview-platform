...
// Створює GracefulShutdown.
export function createGracefulShutdown(dependencies: ShutdownDependencies): (signal: ShutdownSignal) => Promise<void> {
  ...
  return (_signal: ShutdownSignal): Promise<void> => {
    ...
    shutdownPromise = (async () => {
      ...
      await Promise.all([settle(http), settle(socketIo), settle(orchestrator), settle(llm)])
