...
function makeFakePrisma(seed: FakeReport[] | FakePrismaSeed = []) {
  ...
  const applications = Array.isArray(seed) ? [] : [...(seed.applications ?? [])].map(item => ({ ...item }))
  ...
  const prisma = {
    finalReport: {
      ...
      findMany: async ({
        ...
        if (where?.interview?.candidateUser?.email?.contains) {
          ...
          filtered = filtered.filter(r => r.candidateEmail?.toLowerCase().includes(needle) ?? false)
        }
          ...
          ...(include?.interview
            ? {
                interview: {
                  ...
                  ...(include.interview.select.kind ? { kind: report.interviewKind ?? 'STANDARD' } : {}),
                  candidateUser: report.candidateEmail ? { email: report.candidateEmail } : null,
    ...
    interviewDecision: {
      findFirst: async ({
        ...
        if (orderBy?.createdAt === 'desc') {
          matched = [...matched].sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())
        }
      ...
      update: async ({ where, data }: { where: { id: string }; data: { dialogMessageId?: string } }) => {
    ...
    vacancyApplication: {
      findFirst: async ({ where }: { where: { interviewId: string } }) =>
...
test('GET /reports filters by recommendation', async () => {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${port}/api/reports?recommendation=MAYBE`)
    assert.equal(response.status, 200)
    const body = await response.json()
    assert.equal(body.reports.length, 1)
...
test('GET /reports returns 400 for invalid recommendation', async () => {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${port}/api/reports?recommendation=YES`)
...
test('GET /reports filters by vacancyId', async () => {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${port}/api/reports?vacancyId=vac_1`)
    assert.equal(response.status, 200)
    const body = await response.json()
    assert.equal(body.reports.length, 1)
    assert.equal(body.reports[0].id, 'rep_1')
...
test('GET /reports filters by dateFrom and dateTo (UTC day bounds)', async () => {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${port}/api/reports?dateFrom=2026-07-14&dateTo=2026-07-14`)
    assert.equal(response.status, 200)
    const body = await response.json()
    assert.equal(body.reports.length, 1)
    assert.equal(body.reports[0].id, 'rep_1')
...
test('GET /reports returns 400 for invalid dateFrom', async () => {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${port}/api/reports?dateFrom=14-07-2026`)
    assert.equal(response.status, 400)
    const body = await response.json()
...
test('GET /reports/:id returns latest decision when two exist', async () => {
  ...
  const app = makeApp(makeFakePrisma({ reports: [sampleReport], decisions: [older, newer] }), hrUser)
  const server = app.listen(0)
  const port = (server.address() as { port: number }).port
  ...
  try {
    ...
    assert.equal(body.report.latestDecision.createdAt, newer.createdAt.toISOString())
  } finally {
    await new Promise<void>((resolve, reject) => server.close(err => (err ? reject(err) : resolve())))
  }
})
...
test('POST /reports/:id/decisions/draft returns generated letter body', async () => {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${port}/api/reports/rep_1/decisions/draft`, {
      method: 'POST',
...
test('POST /reports/:id/decisions/draft returns 400 without candidateUserId', async () => {
  const app = makeApp(makeFakePrisma([{ ...sampleReport, candidateUserId: null }]), hrUser)
  const server = app.listen(0)
  const port = (server.address() as { port: number }).port
