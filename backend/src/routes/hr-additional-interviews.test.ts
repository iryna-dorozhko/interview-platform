    ...
    interviewDecision: {
      findMany: async ({
        ...
        if (orderBy?.createdAt === 'desc') {
          ...
          matched = [...matched].sort((a, b) => a.createdAt.getTime() - b.createdAt.getTime())
        }
...
function makeApp(fakePrisma: ReturnType<typeof makeFakePrisma>['prisma'], user: AuthUser) {
  ...
  app.use(
    '/api',
    createHrAdditionalInterviewsRouter(() => fakePrisma as never)
  )
  return app
}
...
test('GET /hr/additional-meeting-candidates returns latest additional-meeting decisions per candidate', async () => {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${port}/api/hr/additional-meeting-candidates`)
...
test('POST /hr/interviews/additional returns 400 when vacancy is not confirmed', async () => {
  const { prisma, interviews } = makeFakePrisma({
    users: [{ id: 'cd_1', email: 'c1@example.com', role: 'CANDIDATE' }],
    vacancies: [{ id: 'vac_1', hrUserId: 'hr_1', title: 'Backend', status: 'DRAFT' }],
    interviews: [
      {
        id: 'int_src',
        hrUserId: 'hr_1',
  ...
  try {
    ...
    assert.equal(interviews.filter(item => item.kind === 'ADDITIONAL_MEETING').length, 0)
  } finally {
    await new Promise<void>((resolve, reject) => server.close(err => (err ? reject(err) : resolve())))
  }
})
