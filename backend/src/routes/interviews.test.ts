...
function makeFakePrisma(
  ...
  const prisma = {
    interview: {
      findMany: async ({
        ...
        const filtered = interviews
          .filter(item => {
            ...
            if (displayNameFilter && typeof displayNameFilter === 'object' && 'not' in displayNameFilter) {
        ...
        if (include?.vacancy || include?.finalReport || include?.invitations) {
          return filtered.map(item => {
            ...
            return {
              ...
              ...(include?.vacancy ? { vacancy: { title: vacancy?.title ?? '' } } : {}),
              ...(include?.finalReport
                ? {
                    finalReport: finalReport ? { id: finalReport.id, recommendation: finalReport.recommendation } : null
                  }
                : {}),
              ...(include?.invitations
                ? {
                    invitations: invitations
                      .filter(inv => inv.interviewId === item.id && inv.status === include.invitations!.where.status)
        ...
        if (include?.vacancy || include?.finalReport || include?.invitations) {
          ...
          return {
            ...
            ...(include?.invitations
              ? {
                  invitations: invitations
                    .filter(inv => inv.interviewId === interview.id && inv.status === include.invitations!.where.status)
                    .slice(0, include.invitations!.take)
                }
              : {})
          }
      ...
      update: async ({ where, data }: { where: { id: string }; data: { scheduledAt?: Date | null } }) => {
    ...
    vacancy: {
      findUnique: async ({ where }: { where: { id: string } }) => vacancies.find(v => v.id === where.id) ?? null
    },
    user: {
    ...
    invitation: {
      ...
      findFirst: async ({ where }: { where: { interviewId?: string; status?: string } }) =>
        invitations.find(
          inv =>
...
test('POST /interviews requires vacancyId and CONFIRMED vacancy', async () => {
  const fakePrisma = makeFakePrisma([], [{ id: 'v1', hrUserId: 'hr_1', title: 'Dev', status: 'DRAFT' }])
  const app = makeApp(fakePrisma, { id: 'hr_1', email: 'hr@test.com', role: 'HR' })
  const server = app.listen(0)
  const port = (server.address() as { port: number }).port
...
test('DELETE /interviews/:id cascades and returns 204', async () => {
  ...
  const fakePrisma = makeFakePrisma(interviews, [confirmedVacancy]) as ReturnType<typeof makeFakePrisma> & {
    ...
    interviewDecision: {
      findMany: (args: { where: { interviewId: string }; select: { id: true } }) => Promise<Array<{ id: string }>>
      updateMany: (args: {
    ...
    vacancyApplication: {
      updateMany: (args: { where: { interviewId: string }; data: { interviewId: null } }) => Promise<{ count: number }>
    }
  }
...
function patchInterviewInvitation(port: number, interviewId: string, candidateEmail: string | null) {
...
test('PATCH /interviews/:id rejects ENDED with 409', async () => {
  const fakePrisma = makeFakePrisma([{ ...awaitingInterview, id: 'int_ended', status: 'ENDED' }], [confirmedVacancy])
  const app = makeApp(fakePrisma, { id: 'hr_1', email: 'hr@test.com', role: 'HR' })
  const server = app.listen(0)
  const port = (server.address() as { port: number }).port

  try {
    const response = await patchInterviewSchedule(port, 'int_ended', '2026-07-20T10:00:00.000Z')
    assert.equal(response.status, 409)
    const body = await response.json()
...
test('PATCH invitation when candidate already joined returns 409', async () => {
  const fakePrisma = makeFakePrisma([{ ...awaitingInterview, candidateUserId: 'cand_1' }], [confirmedVacancy])
  const app = makeApp(fakePrisma, { id: 'hr_1', email: 'hr@test.com', role: 'HR' })
  const server = app.listen(0)
  const port = (server.address() as { port: number }).port
