...
// Модуль serializeInvitation.
export function serializeInvitation(inv: { id: string; email: string; status: string } | null | undefined) {
  if (!inv) return null
...
export async function createInterviewWithJoinCode(
  ...
  if (params.kind === 'ADDITIONAL_MEETING') {
    const followUpId =
      typeof params.followUpFromFinalReportId === 'string' ? params.followUpFromFinalReportId.trim() : ''
    if (!followUpId) {
      throw new Error('followUpFromFinalReportId is required for ADDITIONAL_MEETING interviews')
    }
  }
  ...
  for (let attempt = 1; attempt <= MAX_CREATE_ATTEMPTS; attempt++) {
    ...
    try {
        ...
        const interview = await tx.interview.create({
          data: {
            ...
            ...(params.followUpFromFinalReportId
              ...
              : {}),
            ...(params.candidateUserId ? { candidateUserId: params.candidateUserId } : {})
          }
        })
      ...
      const isCandidateUserIdConflict = code === 'P2002' && detail.includes('candidateUserId')
        ...
        fetch('http://127.0.0.1:7331/ingest/5a344c29-d415-4068-bc43-0bba69a8eb6b', {
          ...
          body: JSON.stringify({
            ...
            data: {
              ...
              candidateUserIdSuffix: params.candidateUserId ? params.candidateUserId.slice(-6) : null
            },
            timestamp: Date.now()
          })
...
export function createInterviewsRouter(
  ...
  router.post('/interviews/:id/end', asyncHandler(async (req: Request, res: Response) => {
    ...
    const requirements = normalizeVacancyRequirements(companyProfile.requirements) ?? {
      critical: [],
      desired: []
    }
