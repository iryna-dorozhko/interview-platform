...
test('buildHealthPayload returns ok:true when database and seed are healthy', () => {
  const payload = buildHealthPayload({ ok: true }, { ok: true, email: 'hr@test.com' })
...
test('buildHealthPayload returns ok:false when database fails', () => {
  const payload = buildHealthPayload({ ok: false, error: 'db down' }, { ok: true, email: 'hr@test.com' })
...
test('buildHealthPayload returns ok:false when seed fails', () => {
  const payload = buildHealthPayload({ ok: true }, { ok: false, email: 'hr@test.com' })
