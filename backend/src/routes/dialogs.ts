...
// Модуль hiddenAtFieldForUser.
function hiddenAtFieldForUser(dialog: { hrUserId: string }, userId: string): 'hrHiddenAt' | 'candidateHiddenAt' {
...
// Перевіряє CandidateEligible.
async function isCandidateEligible(prisma: PrismaClient, hrUserId: string, candidateUserId: string): Promise<boolean> {
...
export function createDialogsRouter(getPrisma: () => PrismaClient, getIo: () => Server): Router {
  ...
  router.post('/dialogs', asyncHandler(async (req: Request, res: Response) => {
    ...
    const candidateUserId = typeof req.body?.candidateUserId === 'string' ? req.body.candidateUserId.trim() : ''
    if (!candidateUserId) {
    ...
    const prisma = getPrisma()
    const eligible = await isCandidateEligible(prisma, req.user!.id, candidateUserId)
    if (!eligible) {
  ...
  router.get('/dialogs/unread-count', asyncHandler(async (req: Request, res: Response) => {
    ...
    for (const dialog of dialogs) {
      unreadCount += await countUnreadMessages(prisma, dialog.id, req.user!.id, lastReadAtForUser(dialog, req.user!.id))
    }
  ...
  router.post('/dialogs/:id/read', asyncHandler(async (req: Request, res: Response) => {
    ...
    const now = new Date()
    const data = dialog.hrUserId === req.user!.id ? { hrLastReadAt: now } : { candidateLastReadAt: now }
  ...
  router.post('/dialogs/:id/messages', asyncHandler(async (req: Request, res: Response) => {
    ...
    await prisma.dialog.update({
      ...
      data: {
        updatedAt: new Date(),
        ...(dialog.hrUserId === req.user!.id ? { candidateHiddenAt: null } : { hrHiddenAt: null })
      }
    })
