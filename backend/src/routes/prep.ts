...
// Модуль vacancyHasBlockingInterviews.
...
// Модуль serializeVacancyProfile.
...
export function createPrepRouter(getPrisma: () => PrismaClient, getProvider: () => LlmProvider): Router {
  ...
  router.get('/prep/:vacancyId', async (req: Request, res: Response) => {
    ...
    const profile = session.isClosed ? await prisma.companyProfile.findUnique({ where: { vacancyId } }) : null
