...
function makeFakePrisma(
  ...
  return {
    ...
    prepSessionCandidate: {
      ...
      upsert: async ({ where, create }: { where: { interviewId: string }; create: { interviewId: string } }) => {
    ...
    candidateProfile: {
      ...
      update: async ({ where, data }: { where: { interviewId: string }; data: Record<string, unknown> }) => {
...
test('POST /candidate-prep/:interviewId/confirm transitions to READY when candidate joined', async () => {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${port}/api/candidate-prep/interview_questionnaire/confirm`, {
      method: 'POST'
    })
    assert.equal(response.status, 200)
    const body = await response.json()
