...
// Модуль applicationPayload.
function applicationPayload(application: { id: string; vacancyId: string; matchScore: number; status: string }) {
  return {
    id: application.id,
...
// Створює CandidateMatchesRouter.
export function createCandidateMatchesRouter(getPrisma: () => PrismaClient, getLlmProvider: () => LlmProvider): Router {
  const router = Router()
