  ...
  return {
    ...
    invitation: {
      findMany: async ({
        ...
        include?: {
          interview: { select: { id: boolean; displayName: boolean; scheduledAt: boolean } }
        }
        orderBy?: { createdAt: 'desc' | 'asc' }
      }) => {
        let filtered = invitations.filter(item => item.email === where.email && item.status === where.status)
      ...
      findUnique: async ({ where, include }: { where: { id: string }; include?: { interview: boolean } }) => {
    ...
    interview: {
      ...
      findUnique: async ({
        ...
        return {
          ...
          vacancy: {
            status: 'CONFIRMED',
            companyProfile: include.vacancy?.include?.companyProfile ? { confirmedAt: new Date(1) } : undefined
          },
    ...
    $transaction: async <T>(fn: (tx: ReturnType<typeof makeFakePrisma>) => Promise<T>) =>
      fn(makeFakePrisma(interviews, invitations, options))
  }
}
...
function makeApp(fakePrisma: ReturnType<typeof makeFakePrisma>, _user: AuthUser) {
  ...
  app.use(
    '/api/candidate',
    createCandidateInvitationsRouter(() => fakePrisma as never)
  )
  return app
}
