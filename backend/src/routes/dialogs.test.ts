  ...
  const prisma = {
    dialog: {
      findMany: async ({
        ...
        if (select) {
          return rows.map(d => ({
            ...
            ...(select.candidateLastReadAt ? { candidateLastReadAt: d.candidateLastReadAt } : {})
          }))
        }
        return rows.map(d => {
          ...
          return {
            ...d,
            ...(include?.hrUser ? { hrUser: hrUser ? { id: hrUser.id, email: hrUser.email } : null } : {}),
      ...
      findUnique: async ({
        ...
        if (where.id) {
          ...
          dialog = dialogs.find(d => d.hrUserId === key.hrUserId && d.candidateUserId === key.candidateUserId)
        }
        ...
        if (include?.messages) {
          threadMessages = messages
            ...
            .map(m => {
              const decision = m.decisionId ? decisions.find(d => d.id === m.decisionId) : null
              return {
                ...m,
                ...(include.messages!.include?.decision ? { decision: decision ? { type: decision.type } : null } : {})
              }
            })
        }
    ...
    interview: {
      findFirst: async ({
        ...
        const found = interviews.find(i => i.hrUserId === where.hrUserId && i.candidateUserId === where.candidateUserId)
...
function makeApp(
  ...
  app.use(
    '/api',
    createDialogsRouter(() => fakePrisma as never, getIo as never)
  )
  return app
}
