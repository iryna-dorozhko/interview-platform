...
function makeFakePrisma(seed: {
  ...
  const prisma = {
      ...
      findMany: async ({
        ...
        vacancies
          ...
          .map(item => ({
            ...
            hrUser: include?.hrUser ? (item.hrUser ?? { hrCompanyProfile: null }) : undefined
          })),
    ...
    vacancyMatchScore: {
      findMany: async ({
        ...
        matchScores
          ...
          .map(item => ({
            ...item,
            vacancy: include?.vacancy ? (vacancies.find(vacancy => vacancy.id === item.vacancyId) ?? null) : undefined
          })),
      findFirst: async ({
        where
      }: {
    ...
    vacancyOfferDecision: {
      findMany: async ({ where }: { where: { candidateUserId: string; decision?: string } }) =>
      ...
      create: async ({ data }: { data: { candidateUserId: string; vacancyId: string; decision: string } }) => {
        decisionSeq += 1
    ...
    vacancyApplication: {
      findFirst: async ({ where }: { where: { candidateUserId: string; status?: string } }) =>
      ...
      create: async ({
        ...
        if (
          data.status === 'PENDING' &&
          applications.some(item => item.candidateUserId === data.candidateUserId && item.status === 'PENDING')
        ) {
    ...
    hrNotification: {
      create: async ({ data }: { data: { hrUserId: string; type: string; payload: unknown } }) => {
        notifSeq += 1
...
function confirmedSeed(overrides?: {
  ...
  const profileConfirmedAt = overrides && 'confirmedAt' in overrides ? overrides.confirmedAt : confirmedAt
  return {
    interviews: [
      {
        id: 'q1',
...
function makeApp(fakePrisma: ReturnType<typeof makeFakePrisma>, llm: LlmProvider = makeFakeLlm()) {
  const app = express()
  app.use(express.json())
  app.use(
    '/api/candidate',
    createCandidateMatchesRouter(
      () => fakePrisma as never,
      () => llm
    )
  )
...
test('GET /matches/next returns at most 5 offers sorted by score', async () => {
  ...
  const fakePrisma = makeFakePrisma(confirmedSeed({ vacancies, matchScores }))
  const app = makeApp(fakePrisma)
  const server = app.listen(0)
  const port = (server.address() as { port: number }).port
...
test('POST /candidate/matches/:vacancyId/accept stores matchBreakdown snapshot', async () => {
  ...
  try {
    ...
    assert.equal((application.matchBreakdown as { matchScore: number }).matchScore, 90)
    const body = await response.json()
