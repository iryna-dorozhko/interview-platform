...
async function main(): Promise<void> {
  ...
  try {
    ...
    const sequentialOne = await provider.complete([{ role: 'user', content: `${message} [sequential-1]` }])
    const sequentialTwo = await provider.complete([{ role: 'user', content: `${message} [sequential-2]` }])
