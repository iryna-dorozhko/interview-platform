...
import { createDialog, fetchDialogs, type DialogListItem } from '../api/dialogs'
...
async function loadDialogs(): Promise<void> {
  ...
  try {
    ...
    listError.value = error instanceof Error ? error.message : 'Не вдалося завантажити діалоги'
  }
}
...
async function openNewModal(): Promise<void> {
  ...
  try {
    const [interviews, summaries] = await Promise.all([fetchMyInterviews(), fetchHrApplications()])
    const byId = new Map<string, string>()
    for (const interview of interviews) {
      if (!interview.candidateUserId) continue
      const email = interview.invitation?.email?.trim() || interview.displayName?.trim() || interview.candidateUserId
      if (!byId.has(interview.candidateUserId)) {
        byId.set(interview.candidateUserId, email)
      }
    }
    const applications = await Promise.all(summaries.map(item => fetchHrApplication(item.id)))
    ...
    eligibleError.value = error instanceof Error ? error.message : 'Не вдалося завантажити кандидатів'
  }
}
...
async function submitNewDialog(): Promise<void> {
  ...
  try {
    ...
    createError.value = error instanceof Error ? error.message : 'Не вдалося створити діалог'
  } finally {
  ...
  <div class="dialog-list">
    ...
    <div v-if="showNewModal" class="modal-overlay" @click.self="closeNewModal">
      <div class="modal" role="dialog" aria-labelledby="new-dialog-title">
        ...
        <template v-else>
          <p v-if="eligible.length === 0" class="muted">Немає кандидатів із співбесід або заявок.</p>
