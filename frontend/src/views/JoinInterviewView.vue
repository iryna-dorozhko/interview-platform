...
const redirectPath = computed(() => (code.value ? `/join?code=${encodeURIComponent(code.value)}` : '/join'))
...
async function tryJoin(): Promise<void> {
  ...
  try {
    ...
    errorMessage.value = error instanceof Error ? error.message : 'Не вдалося приєднатися до співбесіди'
  }
}
