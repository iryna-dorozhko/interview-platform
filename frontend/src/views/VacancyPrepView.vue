...
type ArrayProfileField = keyof Pick<
  CompanyProfile,
  'expectations' | 'culture' | 'companyDirection' | 'policies' | 'workFormat' | 'onboardingApproach' | 'workConditions'
>

type VacancySectionId = 'critical' | 'desired' | ArrayProfileField | 'compensation'
...
const isProfileEditable = computed(
  () => !!editableProfile.value && !!profile.value && (!profile.value.confirmedAt || editingConfirmed.value)
)
...
const chat = usePrepChat<CompanyProfile>({
  adapters: {
    loadState: async () => {
      const [vacancy, state] = await Promise.all([fetchVacancy(vacancyId.value), fetchPrepState(vacancyId.value)])
  ...
  shouldAutoGreet: state => !missingCompanyProfile.value && !state.isClosed && state.messages.length === 0,
  onFinished: () => {
    viewingHistory.value = false
  },
  onDeleted: () => {
  ...
  <main class="page">
    ...
    <template v-else>
      ...
      <section v-else-if="isClosed && profile && !viewingHistory" class="profile-view">
        ...
        <p v-if="profile.confirmedAt && !editingConfirmed && vacancyStatus === 'CONFIRMED'" class="confirmed-banner">
          ✓ Анкета підтверджена
        </p>
        <p v-else-if="profile.confirmedAt && !editingConfirmed" class="confirmed-banner">
          ✓ Підтверджено
          {{ profile.confirmedAt ? new Date(profile.confirmedAt).toLocaleString('uk-UA') : '' }}
        </p>
