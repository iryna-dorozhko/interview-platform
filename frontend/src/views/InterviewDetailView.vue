...
async function onSaveSchedule(): Promise<void> {
  ...
  try {
    ...
    actionError.value = error instanceof Error ? error.message : 'Не вдалося оновити час співбесіди'
  } finally {
...
async function onSaveInvitation(): Promise<void> {
  ...
  try {
    ...
    actionError.value = error instanceof Error ? error.message : 'Не вдалося оновити запрошення'
  } finally {
...
async function onCancelInvitation(): Promise<void> {
  ...
  try {
    ...
    actionError.value = error instanceof Error ? error.message : 'Не вдалося скасувати запрошення'
  } finally {
    invitationSaving.value = false
  }
}
...
<template>
  <main class="page">
    ...
    <template v-else-if="interview">
      ...
      <p class="meta">
        Код: <strong class="join-code">{{ interview.joinCode }}</strong> · Статус:
        <strong>{{ statusLabel(interview.status) }}</strong>
      </p>
      ...
      <section v-if="interview.candidateLinked" class="section">
        <h2>Кандидат</h2>
        <p class="hint">
          Кандидат уже привʼязаний до співбесіди (наприклад, з заявки). Email-запрошення не потрібне — він побачить
          зустріч у своєму кабінеті.
        </p>
      </section>
      ...
      <section v-if="canManageInvitation" class="section">
        ...
        <template v-else>
          <label class="field">
            ...
            <input v-model="candidateEmail" type="email" autocomplete="off" :disabled="invitationSaving" />
          </label>
          <div class="actions">
            <button
      ...
      <p v-else-if="interview.status !== 'ENDED'" class="muted">Очікуємо підтвердження профілів від обох сторін.</p>
    </template>
  </main>
</template>
