...
export function useDialogThread(
  ...
  const peerTypingLabel = computed(() => (peerTypingRole.value ? typingLabelFor(peerTypingRole.value) : null))
  ...
  async function loadThread(): Promise<void> {
    ...
    try {
      ...
      loadError.value = error instanceof Error ? error.message : 'Не вдалося завантажити діалог'
    }
  }
  ...
  async function send(): Promise<void> {
    ...
    try {
      ...
      sendError.value = error instanceof Error ? error.message : 'Не вдалося надіслати повідомлення'
    } finally {
      sending.value = false
    }
  }
