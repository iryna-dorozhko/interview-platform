...
import type { AgentThinkingState, ArbiterProcessEntry, LiveAuthorType } from '../composables/useInterviewRoom'
...
<template>
  <aside class="agent-panel" aria-label="Статус AI-агентів">
    ...
    <ul class="agent-list">
      <li v-for="agent in visibleAgents" :key="agent.key" class="agent-item" :class="statusFor(agent.key)">
    ...
    <p v-if="activeAgent" class="panel-hint">Зараз активний: {{ AGENTS.find(a => a.key === activeAgent)?.label }}</p>
