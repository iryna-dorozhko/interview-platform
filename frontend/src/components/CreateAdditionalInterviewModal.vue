...
watch(
  ...
  async isOpen => {
    ...
    try {
      ...
      loadError.value =
        error instanceof Error ? error.message : 'Не вдалося завантажити кандидатів для додаткової зустрічі'
    } finally {
      loading.value = false
    }
  }
    ...
    <div class="modal" role="dialog" aria-labelledby="create-additional-interview-title">
      ...
      <template v-else>
        ...
        <form v-else @submit.prevent="onSubmit">
          ...
          <p v-if="selectedCandidate" class="hint-inline">Вакансія: {{ selectedCandidate.vacancyTitle }}</p>
          <label class="field">
